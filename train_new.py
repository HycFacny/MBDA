...
sys.path.append('../../..')
from modules.pose_MBDA import PoseResNet2d as MBDAPoseResNet, \
    PseudoLabelGenerator2d, PseudoLabelGenerator2dRandomChoice, PseudoLabelGenerator2dHalfbody,  RegressionDisparity
...
def main(args: argparse.Namespace):
    ...
    model = MBDAPoseResNet(backbone, upsampling, 256, num_keypoints, num_advs=args.num_advs, num_head_layers=args.num_head_layers, finetune=True).to(device)
    # model = nn.DataParallel(model, device_ids=[0, 1, 2, 3])
    # define loss function
    criterion = JointsKLLoss()
    criterionMSE = JointsMSELoss()
    criterionCE = torch.nn.CrossEntropyLoss().to(device)
    fgen_method = ['', 'Halfbody', 'RandomChoice', 'Dynamic']
    pseudo_label_generator = eval('PseudoLabelGenerator2d' + fgen_method[args.fgen_method])(
        num_keypoints, args.heatmap_size, args.heatmap_size, args.fgen_param
    )
    regression_disparity = RegressionDisparity(pseudo_label_generator, JointsKLLoss(epsilon=1e-7), num_advs=args.num_advs,
                                               branch_tradeoffs=args.branch_tradeoffs)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_source_iter, train_target_iter, model, criterion, regression_disparity, optimizer_f, optimizer_h, optimizer_h_adv, lr_scheduler_f, lr_scheduler_h, lr_scheduler_h_adv, epoch, visualize if args.debug else None, args)
...
def train(train_source_iter, train_target_iter, model, criterion, regression_disparity,
    ...
    for i in range(args.iters_per_epoch):
        ...
        for j in range(args.num_advs):
            _, a, cnt_s_adv, p = accuracy(y_s_adv[j].detach().cpu().numpy(), label_s.detach().cpu().numpy())
            avg_acc_s_adv.append(a)
            pred_s_adv.append(p)
            acc_s_adv.update(a, cnt_s)
            
            _, a, cnt_t_adv, p = accuracy(y_t_adv[j].detach().cpu().numpy(), label_t.detach().cpu().numpy())
            avg_acc_t_adv.append(a)
            pred_t_adv.append(p)
            acc_t_adv.update(a, cnt_t)
        ...
        if i % args.print_freq == 0:
            ...
            if visualize is not None:
                ...
                visualize(x_s[0], pred_s_adv[0][0] * args.image_size / args.heatmap_size, "source_adv_{}_pred".format(i))
                visualize(x_t[0], pred_t_adv[0][0] * args.image_size / args.heatmap_size, "target_adv_{}_pred".format(i))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='MBDA for Keypoint Detection Domain Adaptation')
    # dataset parameters
    ...
    parser.add_argument('--num-advs', type=int, default=1,
                        help='amount of adversaries')
    parser.add_argument('--branch-tradeoffs', nargs='+', type=float, default=[1.0])
    parser.add_argument('--fgen-method', type=int, default=0)
    parser.add_argument('--fgen-param', type=int, default=0)
